...
/* stored procedures to help for inserting and deleting */
DROP FUNCTION IF EXISTS insert_subscriber(character varying, character varying, character varying, character varying, date);
DROP FUNCTION IF EXISTS insert_competition(character varying, timestamp without time zone, timestamp without time zone, integer, character varying);
DROP FUNCTION IF EXISTS insert_podium_bet(character varying, character varying, integer, integer, integer, integer);
DROP FUNCTION IF EXISTS insert_team(character varying);
DROP FUNCTION IF EXISTS insert_winner_bet(character varying, character varying, integer, integer, integer);
...
***************************************************************/
select insert_subscriber('alice92','alice','rotschield', '123456789', '1992-04-05');
select insert_subscriber('bob_the_king','bobby','sunderland', 'abcdefghij', '1991-03-01');
select insert_subscriber('ahmed_boss','ahmed','sandokan', 'mypasswordis', '1989-02-01');
select insert_subscriber('danyy','daniel','cruis', '1a2b3c4d', '1962-04-05');
select insert_subscriber('manu92snake','manuel','neuer', 'thispassowrd', '1972-04-05');
select insert_subscriber('jacky_sparrow','jack','daniels', 'onemorepass', '1945-04-05');
select insert_subscriber('andrei_ben','andrei','bencsik', 'thebestpassword', '1290-04-05');
select insert_subscriber('leonardt','leonard','toshimori', 'nothisisthebestpass', '1992-04-05');
select insert_subscriber('ahmedsami','ahmed','sami-mohamed', 'bleah', '1990-04-05');
select insert_subscriber('clarasdream','clara','galimberti', '123456789', '1992-04-05');
/***************************************************************
...
select insert_participant(7, 'Primera Division Real-Barcelona');
select insert_participant(8, 'Primera Division Real-Barcelona');

select insert_participant(7, 'Primera Division Real-Barcelona Rematch');
select insert_participant(8, 'Primera Division Real-Barcelona Rematch');

select insert_participant(7, 'Champions league 2016');
select insert_participant(8, 'Champions league 2016');
select insert_participant(10, 'Champions league 2016');

/* place some bets (uname character varying, comp_name character varying, winner_id integer, tokens integer)*/
select insert_winner_bet('andrei_ben', 'Primera Division Real-Barcelona', 8, 100);
select insert_podium_bet('andrei_ben', 'Champions league 2016', 8, 10, 9, 100);
